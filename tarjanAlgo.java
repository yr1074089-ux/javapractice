...
public class tarjanAlgo {
    ...
    public static void createGraph(ArrayList<Edge> graph[]) {
        ...
        graph[4].add(new Edge(4, 5)); // Fixed self-loop to be a triangle for better testing
    ...
    public static void dfs(ArrayList<Edge> graph[], int curr, int par, int dt[], int low[], boolean vis[], int time[]) {
        ...
        for (int i = 0; i < graph[curr].size(); i++) {
            ...
            if (neigh == par) {
                ...
                dfs(graph, neigh, curr, dt, low, vis, time); // FIXED: Pass neigh and curr
    ...
    public static void tarjansBridge(ArrayList<Edge> graph[], int v) {
        int dt[] = new int[v];
        int low[] = new int[v];
        int time[] = {0}; // Using an array to keep time persistent across calls
